...
//   - "vscode-terminal": a fallback used when node-pty can't spawn (e.g. no user-space PTY is available
//                       to the extension process on some macOS builds). The agent is launched in a real
//                       VS Code integrated terminal — which has a working PTY — and revealed so the user
//                       interacts there directly. (Output can't be mirrored back into the webview without
//                       a proposed/blocked API, so the embedded terminal is unused in this mode.)
...
export function spawnAgent(opts: SpawnOptions): SpawnResult {
  ...
  try {
    ...
    // node-pty can't spawn here (e.g. `posix_spawnp failed` on newer macOS — no user-space PTY is
    // available to this process). Delegate to VS Code's own terminal, which has a working PTY, and
    // reveal it so the user interacts there directly. The login+interactive (`-lic`) path is kept so
    // rc-defined PATH (nvm/fnm/npm global bin) is honoured, exactly like the node-pty route.
